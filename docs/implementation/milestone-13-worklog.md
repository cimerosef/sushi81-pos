...
## 2026-09-26 — Catalogue legacy fractional VAT compatibility and bound-ID repair

Consumed `M13-CATALOGUE-VAT-FRACTION-COMPAT-19` from active PR #26 comment `5847492741` while Issue #4 was OPEN. The authorized branch starts at `2f599e213358eece80da389d80c21d577deeeec3`, which already contains the owner-approved catalogue specification and FR/zh-CN guide amendment. Handoff 18 was blocked and superseded; it was not implemented. This package uses synthetic data only and does not touch the owner's A or B profile.

- Preserved percentage-format-based conversion and added only the approved plain-numeric allowlist: decimal `0.055` → `5.5`, `0.1` → `10`, `0.2` → `20`. Decimal equality accepts trailing zeroes. Other plain fractional values remain literal; formula refusal and 0–100 validation remain unchanged.
- Verified the safe bound-ID repair path with a real SQLite test catalogue: current export retains raw legacy values and technical bindings; immediate Update-mode re-import previews only three VAT modifications to the same products, no creates/deletes or child changes; atomic commit preserves IDs, categories, option hierarchy and counts; repaired products produce normal 5.5/10/20 pricing tax buckets.
- Added the narrow B-side owner repair workflow to the FR and zh-CN guides. No schema, reset, order-tax formula, authority/handoff, workbook contract or unrelated UI change was made.

Local verification: focused catalogue workbook integration tests 24/24 passed; full Release suite 932/932 passed with 0 failed/0 skipped; full Release build 0 warnings/0 errors; repository safety scanner self-test 16/16 and full tracked/non-ignored scan 447 files passed; direct/transitive NuGet vulnerability audit passed; `git diff --check` passed. Exact-head CI and a new production-installer artifact remain pending after push. Owner/controller acceptance and real-device repair remain pending; no merge is authorized.
