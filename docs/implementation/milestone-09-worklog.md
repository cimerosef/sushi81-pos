...
### M09 manual-acceptance remediation — payment-only preservation of an active manual total

Status: **Active narrow remediation; owner acceptance paused at checklist F**.

The owner later resumed acceptance on the replacement candidate `a18cca2ad51d3676bc9fc2a99d416ad82a1f1a7a`. Checklists A through E were accepted there. During checklist F, the owner reproduced a payment-only edit defect on synthetic order `20260915-002`: a Hiboutik source total of €35.90, four current €9.50 items, a 15% Retrait discount (calculated POS total €32.30), and an active manual authoritative total of €31.00 were correct before modification. Changing only cumulative CB to €31.00 and saving incorrectly changed the authoritative total back to €32.30 and cleared the manual override.

The controller diagnosis was confirmed in the current code: `OrderDetailLineViewModel.ToSnapshot()` rebuilt an unchanged persisted line from base price and adjustments, omitting the sale-time Retrait discount embedded in `CalculatedLineTotalTtc`. The resulting false line difference made `OrderLifecycleService.SaveModificationAsync` enter its legitimate price-affecting repricing path during an otherwise payment-only edit.
