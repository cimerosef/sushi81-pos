...
### Review cycle 8 — M07 manual-acceptance remediation 19

**Authorized handoff:** `M07-MANUAL-ACCEPTANCE-REMEDIATION-19`; PR #13 top-level handoff comment `5622150776`; Issue #4 was OPEN before execution. The reviewed baseline was `df812ae5a6250e94e33c71203dcdb9e8b083de58`.
**Finding:** `TargetAcquisitionService.DiscoverExactTargetGrantAsync` strictly deserialized every timestamp-shaped `.grant.json` before determining whether the asset belonged to the local current M07 lineage/generation/target. A historical protocol-version-1 grant in the long-lived operational release therefore aborted discovery before the valid current grant could be considered.
**Disposition:** `7fda7d52525f9bad7c77c404afd82d296afea7c1` downloads each candidate once, performs a tolerant envelope/probe classification, ignores malformed/legacy/foreign/non-current evidence that cannot be relevant to the local target, and then applies the existing strict grant, receipt, digest, snapshot and authority-last path to current local evidence. Current-relevant malformed evidence remains fail-closed. Added regression coverage for legacy-before-current, foreign-current-envelope-before-current, malformed-current-relevant and legacy/malformed-only releases. No release asset is deleted or mutated; the existing `WriteAuthorityState`/guard/persistence/recovery path is reused.
**Verification:** Focused `TargetAcquisitionTests`: **9/9 Passed**. Full Release solution: **536/536 Passed, 0 failed, 0 skipped**. `dotnet build Sushi81.Pos.sln -c Release --no-restore`: **0 warnings, 0 errors**. Self-contained `win-x64` publish (`PublishSingleFile=false`) succeeded at `artifacts/m07-manual-remediation-19-publish`; `Sushi81.Pos.Desktop.exe` SHA-256 is `0417961AFF5F3CEC261CEBB6F476B5B2F202DB9BA893EC5A25F9EBA61DE4DC15`.
**Boundaries:** WP7 proof assets/repository were not touched. Owner A remains off and B remains read-only; no real owner/device retest was claimed. M08 and later milestones remain not started, merge remains unauthorized, and project-owner manual acceptance remains pending. Exact-head CI and the durable `CODEX_DONE` record remain to be delivered after push.
