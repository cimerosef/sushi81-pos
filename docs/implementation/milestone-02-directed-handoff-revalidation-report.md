...
The real Home Device A run above supplied transport evidence, but it also demonstrated that the current local observer is a false negative for a remotely-visible regular file (`NO_STATES`). Therefore no documented local-only per-artifact confirmation is available and the technical gate is **Blocked pending an explicit architecture decision**, not Feasible or Partial.
