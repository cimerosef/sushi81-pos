...
## 2026-09-21 — M11-OWNER-A-REPAIR-REAL-WORKBOOK-ROUNDTRIP-11

- Authorization/gate state: Issue #4 was OPEN and dynamically pointed to PR #24 / branch `codex/m11-gestion-export-authorized`; this owner-A repair started at exact Repair 10 head `1732d8b790ad3c45c034c6f6249de3c3140f6ab6`. No merge, M12/M13 work, schema/business-rule change, payment/Close change, or historical business-data rewrite is authorized.
- Reproduction: a production ClosedXML gateway regression used realistic non-zero sub-millisecond CLR ticks in `Meta.GeneratedAt` and `Orders.CreatedAt`. Native Excel/OLE serial conversion drops sub-millisecond ticks, so the previous exact CLR `DateTime` equality rejected the workbook even though its native date values represented the same Excel-contract instant. The regression also verified that native `TimeSpan` precision and two-decimal money/VAT values remain contract-stable.
- Narrow fix: `RequireDateTime` now normalizes the expected value through `DateTime.FromOADate(expected.ToOADate())`, the exact native Excel representable contract, rather than using an arbitrary tolerance. A materially different one-second `CreatedAt` still fails validation. DateOnly remains exact through the same native midnight representation; TimeOnly/TimeSpan remains exact in the existing ClosedXML path; money/VAT remains exact at the fixed native numeric contract.
- Implementation commit: `64c08f8` (`fix: normalize native Excel datetime validation`). Files changed: `src/Sushi81.Pos.Infrastructure/Export/ClosedXmlGestionExportWorkbookGateway.cs` and `tests/Sushi81.Pos.Infrastructure.IntegrationTests/M11Wp2WorkbookGatewayTests.cs`.
- Local evidence: focused M11 workbook-gateway suite **3 passed / 0 failed / 0 skipped**; full Release solution **831 passed / 0 failed / 0 skipped**; Release build **0 warnings / 0 errors**; `git diff --check` clean.
- Acceptance boundary: this repairs the exact production workbook/finalization mismatch and preserves the existing immutable payload, Schema 1.0, eligibility/correction, authority/recovery, and PREPARED BatchId semantics. The prior owner-A failure remains historical evidence; the fresh candidate is **NOT YET RETESTED**, M11 remains not Passed, PR #24 remains unmerged, and M12/M13 remain unauthorized. Exact pushed head, CI and the new owner artifact are recorded in the matching durable `CODEX_DONE` after delivery.

## 2026-09-21 — M11-FINAL-ACCEPTANCE-DOCUMENTATION-CLOSURE-12

- Authorization/gate state: Issue #4 was OPEN and dynamically pointed to PR #24 / branch `codex/m11-gestion-export-authorized`; this documentation/governance-only closure started at exact head `77ccecf9d947462e96e74b8aa1d99ced30e3788e`. No production source, tests, workflow, workbook schema, export semantics, business data or authority/recovery behavior was changed.
- Owner acceptance reconciliation: A = PASS (`5762194270`), B = PASS (`5762229722`), C = PASS (`5762338169`), D = PASS (`5762376235`), E = PASS / A–E summary (`5762475899`). The historical failed A attempts and Repairs 09–11 remain preserved as evidence.
- Accepted runtime candidate: source head `77ccecf9d947462e96e74b8aa1d99ced30e3788e`; exact-head CI #819 / run `35610026160` succeeded; full Release evidence remained 831/831 passed, 0 failed, 0 skipped, with a 0-warning/0-error Release build.
- Accepted artifact: `M11-WP4-owner-candidate-win-x64-77ccecf`, ID `10643522785`, 69,864,230 bytes, expiry `2026-12-20T14:07:30Z`, outer digest `sha256:6a0c53028ae278407d3718e182cb95bceb4390e4f5169d6f587a60e760a6275c`; inner ZIP SHA-256 `B13C6DEC3410C7B9D4C579362940866DAFE2E5A9F6ECF7D19621A300B19C475F`; EXE SHA-256 `4F91EEEFCAFCC1343D86FC03C2F2C4FB83B26A4500A85886990C07BA56017DAC`.
- Status/governance: M11 is **Closure-ready**, not yet Passed, pending final controller closure. PR #24 remains Draft/Open/unmerged; separate explicit project-owner merge approval remains required; M12/M13 remain unauthorized. No further owner manual testing is required for M11.
