...
`control-state-preservation.md` requires interactive actions to preserve unrelated editor/control state unless the frozen workflow explicitly couples those values. For toggles and dynamic editors, tests should cover both true/false preservation across adjacent actions and distinguish transient editor state from persisted Save behavior.
...
- `milestone-03-catalogue-settings.md`
- `milestone-03-filtered-bulk-activation-extension.md`
- `milestone-03-filtered-bulk-activation-authorization.md`
- `milestone-03-worklog.md`

## Current M04 contract

M04 — First complete order-entry vertical slice — is approved for controlled implementation through:

- `milestone-04-order-entry.md` — detailed implementation contract;
- `milestone-04-authorization.md` — durable project-owner authorization;
- `../decisions/m04-order-entry-pricing-clarifications.md` — approved A1/B1/C1 V1 clarification amendment.

M04 must inherit every cross-cutting governance file listed above. Its implementation PR must remain open/unmerged until explicit project-owner merge approval. M04 completion does not authorize M05.

Future milestone contracts M05–M13 must include a short **Parallel execution plan** section that identifies likely independent workstreams and the dependency seams that must be stabilized before concurrent delegation, and must treat `interactive-quality-gate.md` as inherited whenever the milestone includes user-visible UI/workflow behavior.
