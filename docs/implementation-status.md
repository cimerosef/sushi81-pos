...
Focused domain/application/STA-WPF regressions are recorded in `tests/Sushi81.Pos.Domain.Tests/OrderPricingTests.cs`, `tests/Sushi81.Pos.Application.Tests/OrderLifecycleApplicationTests.cs` and `tests/Sushi81.Pos.ArchitectureTests/M05DesktopTests.cs`. Pushed head `9c6cc96e9c8eaa7af1decc895c04fa5643ef1c50` passed locked restore (with approved NuGet network escalation), a 0-warning/0-error Release build, the full 290-test suite with 0 failures/skips, self-contained `win-x64` publish under ignored `artifacts/m05-remediation-publish/`, and GitHub Actions `Continuous integration` run **#294** with Restore, Build and Test successful. This is automated evidence only: project-owner Windows/WPF manual M05 acceptance remains pending, the PR remains open/unmerged, and M06/later milestones remain unauthorized.
