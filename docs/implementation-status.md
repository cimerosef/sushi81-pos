...
- `dotnet test Sushi81.Pos.sln -c Release --no-build`: **196 passed, 0 failed, 0 skipped** (Domain 10; Application 9;
  Infrastructure integration 30; Architecture/localization 23; protocol 32; GitHub wrapper/harness 92, including the
...
directions and refresh, plus All/Active/Inactive preservation and invalid/null fallback. M03 remains Partial solely because the
operator's full Windows/WPF checklist must be rerun after this fix. M04 is not started and is not authorized.
...
- **Manual Windows/WPF verification:** not performed after `M03-MANUAL-UI-STATUS-LIFECYCLE-FIX-06` in this non-interactive
