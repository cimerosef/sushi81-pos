...
- `dotnet test Sushi81.Pos.sln -c Release --no-build`: **197 passed, 0 failed, 0 skipped** (Domain 10; Application 9;
  Infrastructure integration 30; Architecture/localization 24; protocol 32; GitHub wrapper/harness 92, including the
...
in the architecture suite. M03 remains Partial solely because the operator's category-manager and remaining Windows/WPF checklist
must be rerun after this fix. M04 is not started and is not authorized.
...
- **Manual Windows/WPF verification:** not performed after `M03-MANUAL-UI-CATEGORY-LAYOUT-FIX-07` in this non-interactive
  ...
  resize/maximize proportions with readable French/Chinese action buttons, category/product/group/option CRUD and ordering,
