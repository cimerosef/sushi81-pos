...
| M06 — Local recovery/read-only enforcement | In progress | Authorized by `CODEX_HANDOFF_READY: M06-REVIEW-REMEDIATION-04` on PR #11 / `codex/m06-local-recovery-read-only`; fourth controlled review-remediation implementation and evidence are complete, with project-owner Windows/WPF acceptance still pending. |
