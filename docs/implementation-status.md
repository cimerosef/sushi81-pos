...
| M06 — Local recovery/read-only enforcement | In progress | Authorized by `CODEX_HANDOFF_READY: M06-REVIEW-REMEDIATION-02` on PR #11 / `codex/m06-local-recovery-read-only`; review-remediation implementation and evidence are in progress. |
...
| AC-STO-006 | M06 | In progress | M01 snapshot primitive is now wired through the M03–M05 Application mutation boundaries; M06 failure and retention evidence is being recorded on PR #11. |
| AC-STO-007 through AC-STO-009 | M07 | Not started | M02 revalidation prepares target-binding/transport/failure evidence |
| AC-STO-010 | M06 | In progress | M06 durable authority state, centralized write blocking and localized read-only presentation are implemented in progress; M08 printing/M12 archive cross-checks remain deferred. |
