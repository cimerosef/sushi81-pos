...
| M02 — remote handoff feasibility gate | In progress / Partial | Original competitive OneDrive model remains historically Blocked. Approved GitHub private Release Asset transport is implemented with automated receipt/failure evidence; real private-repository A→B v1 and B→A v2 evidence is still required. |
...
**Historical directed revalidation status:** Implemented correction pass; its OneDrive-only transport conclusion is superseded by the approved GitHub transport amendment below. The current gate is Partial pending real private-repository evidence.
...
**Status:** `Partial — GitHub transport implementation ready; real two-device private-repository evidence required`
...
The implementation adds direct .NET `HttpClient` GitHub Release Asset transport against a configurable dedicated private repository. The source flow requires HTTP 201, `uploaded`, exact name/size, asset ID and matching `sha256:<hex>` receipt for the snapshot before durable relinquishment; it then uploads/validates the exact-basename grant before `Released`. The target flow validates grant metadata, exact referenced asset identity including source device, authenticated download, local hash/size and SQLite integrity before the existing pending/evidence/final-cursor write gate. The GitHub target wrapper reconstructs the device-local source state so a returning device can validate its exact Released predecessor for B → A v2 and A → B v3. A valid receipt from an earlier completed transfer is ignored for a later transfer, while malformed or same-transfer receipts still fail closed. Retention validates complete snapshot+grant units release-wide and retains exactly the newest three by generation/handoff-version metadata; cleanup failure is retryable and cannot roll authority back. Grant upload failure and pre-`Released` commit interruption are restart-safe and do not duplicate an acknowledged asset. Credentials are read only from `SUSHI81_GITHUB_HANDOFF_TOKEN` and are not serialized/logged. Correction implementation head: `5402c50cc528d70297946e2281bb86eeef6c0ea6`; the final pushed branch-head CI result is recorded in PR #3 checks.

The current automated GitHub transport evidence is synthetic/fake-HTTP only: the latest correction pass records 150 passed, 0 failed and 0 skipped across the solution (Domain 3, Application 2, Infrastructure integration 16, Architecture 8, protocol 32, GitHub wrapper/harness 89), with 0 Release build warnings/errors and a self-contained win-x64 publish. Coverage includes A → B v1 → B → A v2 → A → B v3 through the GitHub wrappers, stale-receipt replacement, release-wide retention, grant restart/retry, target truncation/hash/SQLite corruption, duplicate-name 422, upstream 502/starter and timestamp collision. No live GitHub repository, token, database, snapshot or business data is committed. The report also preserves sanitized real historical Home Device A OneDrive evidence; that OneDrive `IN_SYNC` observation is historical only. Real operator evidence still requires private repository setup plus sequential A → B v1 and B → A v2, restart/resume, target filtering and retention checks. M03 remains Not started.
