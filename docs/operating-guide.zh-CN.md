...
XLSX 的 `Products` 表中，VAT 使用百分比点数值：普通数字 `5.5` 表示 5.5%，`10` 和 `20` 同理。设置为 Excel 百分比格式、显示为 `5.5%`、`10%` 或 `20%` 的数字单元格也可导入，系统会换算为相同税率。提交前请在导入预览中核对代表性税率行，尤其是来自外部的工作簿；未设置百分比格式的普通数字 `0.055` 按字面解释为 0.055%，不会当成 5.5%。
