...
XLSX 的 `Products` 表中，VAT 使用百分比点数值：普通数字 `5.5` 表示 5.5%，`10` 和 `20` 同理。设置为 Excel 百分比格式、显示为 `5.5%`、`10%` 或 `20%` 的数字单元格也可导入，系统会换算为相同税率。为兼容 Sushi81 旧商品目录文件，普通数字中的三个明确旧编码也会被规范化：`0.055` → `5.5`、`0.1` / `0.10` → `10`、`0.2` / `0.20` → `20`。这只是封闭的兼容清单，不是“所有小于 1 的数字都乘 100”的规则；其他普通小数（例如 `0.15`、`0.5`、`0.075`）仍按字面百分比点解释。提交前请在导入预览中核对代表性税率行。

若当前具有写入权限的 B 已导入上述旧税率，请在保留 B 现有配置的前提下安装修复版，从 B 导出**当前**商品目录，然后立即以“更新”模式导回这份原样导出的文件。提交前确认预览中只有同一批现有商品的 VAT 更新，没有新增或删除；提交后核对代表性的 5.5/10/20 税率及商品、分类、选项的数量和身份。确认修复成功后，若需同步 A，再按正常交接流程操作。本次修复不需要手动修改 SQLite 或重置目录。
