...
internal sealed class CatalogueImportPreviewDialog : Window
{
    ...
    private StackPanel BuildIssues()
    {
        ...
        var grid = new DataGrid { ItemsSource = CatalogueImportIssuePresenter.Present(preview.Preview.Issues, localized), AutoGenerateColumns = false, IsReadOnly = true, CanUserAddRows = false, Height = 190, MinHeight = 90, HeadersVisibility = DataGridHeadersVisibility.Column, RowHeight = double.NaN };
        AddColumn(grid, "Severity", "SeverityText", 95);
        AddColumn(grid, "Worksheet", "Worksheet", 120);
        AddColumn(grid, "Row", "Row", 58);
        AddColumn(grid, "Field", "Field", 120);
        AddColumn(grid, "Message", "Message", 480, wrap: true);
        var panel = new StackPanel();
    ...
    private StackPanel BuildAffectedRows()
    {
        ...
        var grid = new DataGrid { ItemsSource = workflow.AffectedRows, AutoGenerateColumns = false, IsReadOnly = true, CanUserAddRows = false, Height = 130, MinHeight = 70 };
        AddColumn(grid, "Worksheet", "Worksheet", 120);
        AddColumn(grid, "Row", "ExcelRow", 70);
        AddColumn(grid, "Entity", "EntityType", 120);
        AddColumn(grid, "Actions", "Actions", 300, wrap: true);
        var panel = new StackPanel();
        panel.Children.Add(heading);
        panel.Children.Add(grid);
        return panel;
    ...
    private async void ConfirmAsync(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            if (!result.Succeeded)
            {
                status.Text = string.Join(Environment.NewLine, CatalogueImportIssuePresenter.Present(result.Issues, localized).Select(issue => issue.Message));
                closeAllowed = false;
                return;
            }

            if (!result.Changed)
                status.Text = Read(localized, "CatalogueImportNoChange", "No Catalogue change was needed.");
            else
                status.Text = Read(localized, "CatalogueImportSucceeded", "Catalogue import succeeded.");
            closeAllowed = true;
            DialogResult = true;
        }
        catch (Exception exception)
        {
            status.Text = Read(localized, "CatalogueImportRefreshFailed", "Import succeeded, but the interface refresh failed; restart or refresh before continuing.");
            status.Text += Environment.NewLine + exception.Message;
            closeAllowed = false;
        }
        finally { SetActionState(workflow.CanConfirm); }
    }

    private void SetActionState(bool enabled)
    {
        confirm.IsEnabled = enabled && workflow.CanConfirm;
